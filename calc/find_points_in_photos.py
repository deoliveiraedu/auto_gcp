...
def main():
    _no_result = {'total_gcps': 0, 'found_count': 0, 'total_entries': 0, 'photos_count': 0}
    ...
    # Detectar projeção do gcp_list.txt (gerado pelo converter a partir do CSV)
    ...
    transformer = Transformer.from_crs("epsg:4326", proj_line, always_xy=True)

    # Count photos to scan
    all_photos = []
    for root, _, files in os.walk(config.PHOTO_DIR):
        for f in sorted(files):
            if f.lower().endswith(('.jpg', '.jpeg')):
                all_photos.append(os.path.join(root, f))
    print(f"      Analisando {len(all_photos)} fotos...")

    photos_data = {}

    for img_path in all_photos:
    ...
    empty_stats = {'total_gcps': len(gcps), 'found_count': 0, 'total_entries': 0, 'photos_count': 0}
    if not photos_data:
        return empty_stats
    ...
    os.makedirs(os.path.dirname(config.GCP_LIST_IM), exist_ok=True)
    ...
    found_gcp_ids = set(item['gcp_id'] for item in final_list)
    return {
        'total_gcps': len(gcps),
        'found_count': len(found_gcp_ids),
        'total_entries': len(final_list),
